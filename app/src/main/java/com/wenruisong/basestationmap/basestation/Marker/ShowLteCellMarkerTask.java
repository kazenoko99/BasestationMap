...
public class ShowLteCellMarkerTask implements Runnable {
    ...
    public synchronized void showLteCellMarkers() {
        ...
        if (ms.zoom >= 17) {
            ...
            Cursor cursor = BasestationManager.basestationDB.rawQuery("select CELLINDEX from lte_cells where baidulat>? and baidulat<? and BAIDULON>? and BAIDULON<? group by BS,LAT,LON,azimuth", new String[]{minlat, maxlat, minlng, maxlng});
            cursor.moveToFirst();
            cellCount = cursor.getCount();
            int count = Math.min(cellCount, maxShowCells);
            for (int i = 0; i < count; i++) {
                Cell cell = BasestationManager.lteCells.get(cursor.getInt(0));
                CellMarker cellMarker = lteCellsToShow.get(cell);
                if (cellMarker == null) {
                    if (cell.type == 1) {
            ...
            cursor.close();
            cursor = BasestationManager.basestationDB.rawQuery("select CELLINDEX from lte_cells where baidulat>? and baidulat<? and BAIDULON>? and BAIDULON<? group by BS,LAT,LON", new String[]{minlat, maxlat, minlng, maxlng});
            cursor.moveToFirst();
            count = Math.min(cursor.getCount(), maxShowBsName);
            for (int i = 0; i < count; i++) {
                Basestation basestation = BasestationManager.lteBSs.get(cursor.getInt(0));
            ...
            lastZoomLevel = ms.zoom;
            Cursor cursor = BasestationManager.basestationDB.rawQuery("select CELLINDEX from lte_cells where baidulat>? and baidulat<? and BAIDULON>? and BAIDULON<? and TYPE=0 group by BS,LAT,LON", new String[]{minlat, maxlat, minlng, maxlng});
            cursor.moveToFirst();
            cellCount = cursor.getCount();
