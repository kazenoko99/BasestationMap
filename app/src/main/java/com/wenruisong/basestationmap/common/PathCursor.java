...
public class PathCursor extends AbstractCursor {
    ...
    public static final String CN_IS_CSV = "is_CSV";
    public static final String[] columnNames = new String[]{CN_ID, CN_FILE_NAME, CN_FILE_PATH, CN_IS_DIRECTORY, CN_IS_CSV};
    ...
    public static final int CI_IS_CSV = 4;

    PathCursor(File parentDirectory, File[] fileList) {
        if (parentDirectory.getParent() != null) {
            FileItem parentFile = new FileItem(new File(parentDirectory, "..返回上一级目录"));
        ...
        if (fileList != null) {
            for (File file : fileList) {
                if(!".".equals(file.getName().charAt(0))) {
                    FileItem fileItem = new FileItem(file);
                    if (fileItem.isCSV || fileItem.isDirectory)
    ...
    public long getLong(int column) {
        switch (column) {
            ...
            case CI_IS_CSV:
                return mFileList.get(getPosition()).isCSV ? 1 : 0;
        }
        return 0;
    }
    ...
    private class FileItem {
        ...
        public FileItem(File file) {
            this.file = file;
            if(file.isDirectory()) {
                Character character = file.getName().charAt(0);
                this.isDirectory = character.equals('.')? false:true;
            }
