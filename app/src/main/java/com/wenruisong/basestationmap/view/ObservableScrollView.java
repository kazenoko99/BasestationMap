...
public class ObservableScrollView extends ScrollView implements Scrollable {
    ...
    private MotionEvent mLiveMotionEvent;
    private boolean mbEnableCheckStop = false;
    private static final long mDelayMills = 120;
    ...
    private void checkIfStopped() {
        ...
        mTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                if (mLiveMotionEvent != null && mLiveMotionEvent.getAction() == MotionEvent.ACTION_UP) {
                    if (mCallbacks != null) {
                        mCallbacks.onScrollStopped(getScrollY());
                        clearTimer();
                    }
                } else {
                    checkIfStopped();
    ...
    public void setCheckStopEnabled(boolean check) {
        this.mbEnableCheckStop = check;
        if (!check) {
            clearTimer();
        }
    }
    ...
    public void removeScrollViewCallbacks(ObservableScrollViewCallbacks listener) {
        if (listener != null && listener.equals(mCallbacks)) {
            mCallbacks = null;
        }
    }
