package com.wenruisong.basestationmap.tools.camera;
...
public class CameraPreview extends SurfaceView implements
	...
	private Camera.Size findPreviewSizeByScreen(Camera.Parameters parameters) {
		if (viewWidth != 0 && viewHeight != 0) {
			...
			return camera.new Size(DeviceUtils.getScreenWH(getContext()).heightPixels,
					DeviceUtils.getScreenWH(getContext()).widthPixels);
		}
	}
	...
	private Camera.Size findBestPreviewSize(Camera.Parameters parameters) {
		...
		if (previewSizeValueString == null) { // 有些手机例如m9获取不到支持的预览大小 就直接返回屏幕大小
			return camera.new Size(DeviceUtils.getScreenWH(getContext()).widthPixels,
					DeviceUtils.getScreenWH(getContext()).heightPixels);
		}
		float bestX = 0;
		float bestY = 0;
	...
	public void focusOnTouch(MotionEvent event) {
		...
		Rect focusRect = DeviceUtils.calculateTapArea(mFocusView.getWidth(),
		...
		Rect meteringRect = DeviceUtils.calculateTapArea(mFocusView.getWidth(),
	...
	public void setFocus() {
		if(!mFocusView.isFocusing()) {
			try {
				camera.autoFocus(this);
				mFocusView.setX((DeviceUtils.getWidthInPx(getContext())-mFocusView.getWidth()) / 2);
				mFocusView.setY((DeviceUtils.getHeightInPx(getContext())-mFocusView.getHeight()) / 2);
				mFocusView.beginFocus();
			} catch (Exception e) {
			}
		}
